...
func on_physics_process(delta):
	...
	if player.wall_controller_r.is_colliding() and player.wall_controller_rc.is_colliding() or player.wall_controller_l.is_colliding() and player.wall_controller_lc.is_colliding():
	...
	if !player.wall_controller_r.is_colliding() and not player.wall_controller_rc.is_colliding() and !player.wall_controller_l.is_colliding() and not player.wall_controller_lc.is_colliding():
...
func on_input(_event):
	if Input.is_action_just_pressed("jump") and player.wall_controller_l.is_colliding() and player.wall_controller_lc.is_colliding():
		controlled_node.velocity.x = player.movement_stats.wall_jumpHL_force
		state_machine.change_to(player.states.Fall)
	elif Input.is_action_just_pressed("jump") and player.wall_controller_r.is_colliding() and player.wall_controller_rc.is_colliding():
		controlled_node.velocity.x = player.movement_stats.wall_jumpH_force
		state_machine.change_to(player.states.Fall)
	if player.wall_controller_l.is_colliding() and player.wall_controller_lc.is_colliding():
		if Input.is_action_just_pressed("right"):
			print(player.movement_stats.jump_wall_actived)
			state_machine.change_to(player.states.WallJump)
	elif  player.wall_controller_r.is_colliding() and player.wall_controller_rc.is_colliding():
